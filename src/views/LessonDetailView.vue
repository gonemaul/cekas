...
<template>
  <div v-if="lesson" class="space-y-6 animate-in fade-in zoom-in-95 duration-300">
    <button @click="router.push('/learn')" class="flex items-center gap-2 text-slate-400 font-bold text-sm hover:text-primary transition-colors">
      <ArrowLeftIcon class="w-4 h-4" /> KEMBALI KE KURIKULUM
    </button>

    <div class="bg-primary p-8 rounded-[2.5rem] text-white shadow-xl shadow-blue-100 relative overflow-hidden">
      <div class="relative z-10">
        <span class="text-[10px] font-black bg-white/20 px-3 py-1 rounded-full uppercase tracking-widest">
    ...
    <div class="space-y-4">
      <div class="bg-white p-6 rounded-3xl border-2 border-slate-100">
        <h4 class="text-xs font-black text-primary uppercase tracking-widest mb-3 flex items-center gap-2">
  ...
  <p class="text-xs text-slate-500 italic leading-relaxed">
    {{ lesson.tips }}
      ...
      <div class="bg-emerald-50 p-6 rounded-3xl border-2 border-emerald-100 relative">
        ...
        <div class="absolute top-4 right-6 text-emerald-200 uppercase font-black text-xs italic">Example</div>
      </div>
    </div>
  ...
  <div v-else class="text-center py-20">
    ...
    <button @click="router.push('/learn')" class="text-primary font-black mt-4 underline">Kembali</button>
  </div>
</template>